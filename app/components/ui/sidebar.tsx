  ...
  return (
    <SidebarContext.Provider value={{ state, open, onOpenChange }}>
      <div
        ...
        className={cn(
          'flex max-h-full h-full bg-accent/50 pl-0 transition-all duration-300 ease-in-out data-[sidebar=closed]:pl-0 sm:pl-[--sidebar-width]',
          className
        )}
        {...props}
      />
...
const Sidebar = React.forwardRef<HTMLDivElement, React.ComponentProps<'div'>>(
  ({ className, children }, ref) => {
    ...
    return (
      <aside className="fixed inset-y-0 left-0 z-10 hidden w-[--sidebar-width] transition-all duration-300 ease-in-out md:block h-full [[data-sidebar=closed]_&]:left-[calc(var(--sidebar-width)*-1)]">
        {sidebar}
      </aside>
    );
  }
